...
# The Korg.jl release this port targets. Must match the pin in Project.toml.
# Reference data generated by any other version is a stale-fixture bug, not a
# physics disagreement, so we surface it as its own failure.
EXPECTED_KORG_VERSION = "1.2.1"
...
def test_reference_data_korg_version(reference_data):
    """The reference fixtures must come from the pinned Korg.jl release."""
    metadata = reference_data.get("metadata")
    assert metadata is not None, (
        "julia_reference_data.json has no 'metadata' block — it predates version "
        "stamping. Regenerate with: julia --project=. tests/generate_julia_reference.jl"
    )
    assert metadata["korg_version"] == EXPECTED_KORG_VERSION, (
        f"Reference data was generated by Korg.jl v{metadata['korg_version']}, but this "
        f"port targets v{EXPECTED_KORG_VERSION}. Regenerate with: "
        "julia --project=. tests/generate_julia_reference.jl"
    )
...
class TestChemicalEquilibriumReference:
    ...
    def test_hminus_is_returned(self, reference_data):
        """H- is carried by chemical equilibrium (new in Korg.jl v1.2)."""
        if "chemical_equilibrium" not in reference_data:
            pytest.skip("Chemical equilibrium reference data not available")
        ...
        assert np.isclose(number_densities[H_minus], expected, rtol=1e-10)

        # Sanity: H- is a trace species but not negligible at solar temperatures.
        assert 0 < number_densities[H_minus] < number_densities[Species.from_string("H I")]

    def test_cool_dense_layer_converges(self):
        """The molecular continuation must handle the cool, dense regime.

        At T ~ 2500 K and n_total ~ 1e17 an atoms-only initial guess massively
        over-predicts molecular densities, so the direct Newton solve fails and the
        solver has to anneal molecules back on. This is the case that motivated the
        ...
        n_H_I = number_densities[Species.from_string("H I")]
        n_H2 = number_densities[Species.from_string("H2")]
        assert np.isfinite(n_H_I) and n_H_I > 0
        # At 2500 K molecular hydrogen is a significant reservoir, so a solution that
        # ignored molecular depletion would put essentially all hydrogen in H I.
        assert n_H2 > 0.01 * n_H_I, f"H2/H I = {n_H2 / n_H_I:.3e} — molecules look suppressed"
