...
def _initialize_Hminus_ff_interpolator():
    """
    Initialize H⁻ free-free absorption interpolator.

    Uses the table from Bell & Berrington (1987), which tabulates
    the H⁻ free-free absorption coefficient K (including stimulated emission)
    as a function of wavelength and temperature.

    Returns
    -------
    interpolator : RegularGridInterpolator
        2D interpolator for K(λ, θ) where θ = 5040/T
    """
    global _Hminus_ff_interpolator

    if _Hminus_ff_interpolator is not None:
        return _Hminus_ff_interpolator

    # Table from Bell & Berrington (1987)
    # https://doi.org/10.1088/0022-3700/20/4/019

    # Temperature parameter: θ = 5040/T
    theta_grid = np.array([0.5, 0.6, 0.8, 1.0, 1.2, 1.4, 1.6, 1.8, 2.0, 2.8, 3.6])
    ...
    # Create interpolator
    # Note: RegularGridInterpolator expects (lambda, theta) order
    _Hminus_ff_interpolator = RegularGridInterpolator(
        (lambda_grid, theta_grid),
        ff_table,
        method='linear',
        bounds_error=True
    )
...
def Hminus_ff(nu, T, nH_I_div_partition, ne):
    ...
    interpolator = _initialize_Hminus_ff_interpolator()
    ...
    theta = np.clip(theta, 0.5, 3.6)

    # Interpolate K (in units of 1e-26 cm^4/dyn, factor built into table)
    # Create points array for interpolation
    points = np.column_stack([lambda_angstrom, np.full_like(lambda_angstrom, theta)])
    K = 1e-26 * interpolator(points)
