...
# --- chemical_equilibrium in isolation ---
print("\n--- chemical_equilibrium (statmech) ---", flush=True)
T = 5777.0
n_total = 1e17
ne_model = 3e13
...
t0 = time.perf_counter()
ne, nd = chemical_equilibrium(T, n_total, ne_model, abs_abund, ion_e, pf, log_Keq)
print(f"  1st call: {time.perf_counter()-t0:.3f}s  (ne={ne:.3e})", flush=True)

t0 = time.perf_counter()
ne, nd = chemical_equilibrium(T*1.01, n_total, ne_model, abs_abund, ion_e, pf, log_Keq)
print(f"  2nd call: {time.perf_counter()-t0:.3f}s", flush=True)

t0 = time.perf_counter()
ne, nd = chemical_equilibrium(T*0.99, n_total*2, ne_model, abs_abund, ion_e, pf, log_Keq)
print(f"  3rd call: {time.perf_counter()-t0:.3f}s", flush=True)

# Time 10 calls to get stable estimate
t0 = time.perf_counter()
for i in range(10):
    ne, nd = chemical_equilibrium(T*(1+0.01*i), n_total, ne_model, abs_abund, ion_e, pf, log_Keq)
